...
def cmd_test(_args):
    test_dir = ROOT.parent / "tests" if (ROOT.parent / "tests").exists() else ROOT / "tests"
    print("Running pytest suite...")
    ret = subprocess.run([sys.executable, "-m", "pytest", str(test_dir), "-v"], cwd=str(ROOT.parent if (ROOT.parent / "tests").exists() else ROOT))
    if ret.returncode != 0:
        sys.exit(ret.returncode)

    parity_script = test_dir / "parity" / "run_parity.py"
    print("\nRunning Cross-Language Parity Check...")
    ret_parity = subprocess.run([sys.executable, str(parity_script)], cwd=str(ROOT.parent if (ROOT.parent / "tests").exists() else ROOT))
...
def cmd_parity(_args):
    test_dir = ROOT.parent / "tests" if (ROOT.parent / "tests").exists() else ROOT / "tests"
    parity_script = test_dir / "parity" / "run_parity.py"
    ret = subprocess.run([sys.executable, str(parity_script)], cwd=str(ROOT.parent if (ROOT.parent / "tests").exists() else ROOT))
    sys.exit(ret.returncode)
